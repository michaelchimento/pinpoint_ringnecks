...
target_pops = ["P2","P3","P4","P5","P6"]

types = ["Social1","Social2","Social3","Feeder"]
...
for camera_type in types:
    for target_pop in target_pops:
        ...
        df_list = [pd.read_csv(join(folder,f)) for folder in folder_list for f in listdir(folder) if re.search("P\d+", folder).group(0)==target_pop and camera_type in folder]
        
        if len(df_list) > 0:
            df_concat = pd.concat(df_list)
            df_concat = df_concat.drop_duplicates(subset = ["population","room","time","id"],keep='first')
